...
function copyModFiles(){
  return gulp.src(['common/**/*', 'events/**/*', 'localization/**/*', 'descriptor.mod', 'thumbnail.jpg'], {base: '.'})
  .pipe(bom())
  .pipe(gulp.dest('../build/'))
}

function copyGFX(){
  return gulp.src(['gfx/**/*'], {base: '.'})
  .pipe(gulp.dest('../build/'))
}

const build = gulp.series(clean, cloneLangs, copyModFiles, copyGFX);
